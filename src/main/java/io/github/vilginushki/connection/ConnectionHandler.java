...
public class ConnectionHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        bundle = map.get(IPWrapper.convert(ip));
        if (bundle == null) {
        ...
        log.info("ChannelActive with: " + IPWrapper.convert(bundle.getIp()) + "/" + bundle.getPort());
        ...
        switch (typeOfAction) {
            case GETADDR -> {
                switch (message.getCommand()) {
                    case VERSION -> {
                        log.info("Got: VERSION; From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new Verack().serialize());
                        bundle.setVersion(new Version(message.getHeader()));
                    }
                    case VERACK -> {
                        log.info("Got: VERACK; From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new GetAddr().serialize());
                        isSendGetAddr = true;
                    }
                    case ADDR -> {
                        log.info("Got: ADDR; From " + IPWrapper.convert(bundle.getIp()));
                    ...
                    case PING -> {
                        ...
                        log.info("Got: PING(" + ping.getTemp() + "); From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new Pong(ping.getTemp()).serialize());
                        log.info("Sent: PONG; TO:" + IPWrapper.convert(bundle.getIp()));
                    }
                    case REJECT -> {
            ...
            case SCAN -> {
                switch (message.getCommand()) {
                    case VERSION -> {
                        log.debug("Got: VERSION; From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new Verack().serialize());
                        bundle.setVersion(new Version(message.getHeader()));
                    }
                    case VERACK -> {
                        log.debug("Got: VERACK; From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new GetAddr().serialize());
                        isSendGetAddr = true;
                    }
                    case ADDR -> {
                        log.debug("Got: ADDR; From " + IPWrapper.convert(bundle.getIp()));
                        Addr addr = new Addr(message.getHeader());
                        if (isSendGetAddr && addr.getList().size() > 1) {
                    ...
                    case PING -> {
                        Ping ping = new Ping(message.getHeader());
                        log.debug("Got: PING(" + ping.getTemp() + "); From " + IPWrapper.convert(bundle.getIp()));
                        writeAndFlush(ctx, new Pong(ping.getTemp()).serialize());
                        log.debug("Sent: PONG; TO:" + IPWrapper.convert(bundle.getIp()));
                    }
                    case REJECT -> {
                        Reject reject = new Reject(message.getHeader());
                        log.info("GOT: REJECT" + reject);
            ...
            case PING -> {
                switch (message.getCommand()) {
                    ...
                    case VERACK -> {
                        ...
                        log.info("Sent: Ping(" + tmp.getTemp() + ") to: " + IPWrapper.convert(bundle.getIp()));
                    }
                    case ADDR -> log.info("Got: ADDR; From " + IPWrapper.convert(bundle.getIp()));
                    case PONG -> {
                        Pong tmp2 = new Pong(message.getHeader());
                        log.info("Got: PONG(" + tmp2.getTempValue() + ") from: " + IPWrapper.convert(bundle.getIp()));
                        ctx.close();
                    }
                    case REJECT -> {
                        Reject reject = new Reject(message.getHeader());
            ...
            default -> {
                log.info("Got: " + message.getCommand() + "; From " + IPWrapper.convert(bundle.getIp()));
                ctx.close();
            }
